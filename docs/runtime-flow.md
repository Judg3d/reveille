...
```text
/_reveille/wait?host=<app-host>&returnTo=<original-path>&token=<wait-token>
```

The relative redirect stays on the same public host so the browser can reach it
through Traefik. The signed wait token is generated by Reveille, expires after
24 hours, and is bound to the host from the `forwardAuth` request.
