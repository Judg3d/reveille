...
The redirect uses the public managed host so the browser can reach it through
Traefik instead of the internal Reveille service name. The signed wait token is
generated by Reveille, expires after 24 hours, and is bound to the host from the
`forwardAuth` request.
